...
class AppToolbar extends React.Component<MappedDispatchProps & MappedStateProps & Props, State> {
  ...
  private renderActions = (user: UserDispatchProps) => {
    if (user.isFetching) {
      return <button className="btn btn-dark">Loading...</button>;
    } else if (user.user) {
      return (
      ...
      return <button className="btn btn-danger">Error</button>;
    } else {
      return <div />;
    }
  };
  ...
  public render() {
    return (
      <nav className="navbar navbar-light bg-light">
        ...
        {this.renderActions(this.props.user)}
      </nav>
    );
  }
}
